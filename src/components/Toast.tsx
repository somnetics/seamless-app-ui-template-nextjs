...
type ToastContextType = {
  addToast: (message: string, type: MessageTypes, timeout?: number) => void;
};
...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  const addToast = (message: string, type: MessageTypes, timeout?: number) => {
    const id = randomId();
  ...
  const hideToast = (id: string) => {
    ...
    }, 250); // match with Tailwind transition duration
  };
      ...
      <div className="flex flex-col items-center justify-center w-full fixed top-5 z-1000 gap-2">
        {toasts.map((toast) => (
          <div key={toast.id} className={"flex items-center w-full relative max-w-md p-4 text-gray-500 bg-white rounded-lg shadow-sm dark:text-gray-400 dark:bg-gray-800 transition-opacity duration-250 ease-in-out " + (toast.visible ? "opacity-100" : "opacity-0")} role="alert">
            {toast.type == MessageTypes.Success ? (
            ...
            ) : toast.type == MessageTypes.Warning ? (
              ...
              <div className="inline-flex items-center justify-center shrink-0 w-8 h-8 text-blue-500 bg-blue-100 rounded-lg dark:bg-blue-800 dark:text-red-200">
                <svg className="w-5 h-5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                  <path d="M10 .5a9.5 9.5 0 1 0 9.5 9.5A9.51 9.51 0 0 0 10 .5ZM10 15a1 1 0 1 1 0-2 1 1 0 0 1 0 2Zm1-4a1 1 0 0 1-2 0V6a1 1 0 0 1 2 0v5Z" />
                </svg>
