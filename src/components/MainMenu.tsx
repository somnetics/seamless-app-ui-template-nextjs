...
export default function MainMenu() {
  ...
  const handleLogout = async (e: any) => {
    ...
    setTimeout(() => {
      ...
      // show message
      addToast(data.message, MessageTypes.Success, 5000);
  ...
  return (
    <div className={"h-full transition-[width] duration-100 ease-in-out sm:sticky sm:left-0 sm:top-0 sm:w-16 hidden sm:block " + (open ? "xl:w-[250px]" : "xl:w-16")}>
            ...
            {open && (
              <div className="pl-1 sm:hidden xl:block">
                <a className="text-surface-accent-0 dark:text-white flex gap-2 items-start justify-center" aria-label="Freepik" href="#">
                  ...
                  <span className="inline-flex items-center rounded-md bg-purple-400/10 px-2 py-1 text-xs font-medium text-purple-400 inset-ring inset-ring-purple-400/30">v4.0</span>
                </a>
              </div>
            )}
            <button aria-label="Toggle sidebar" onClick={() => setOpen(!open)} className="cursor-pointer flex items-center rounded-lg hover:bg-gray-200 dark:hover:bg-gray-700 p-[9px] sm:hidden xl:flex">
          ...
          <SimpleBar className="relative flex-1 pl-4 pr-2 overflow-auto" style={{ height: 1 }}>
            <div className="size-full pr-2">
              ...
              <div className="space-y-1">
                {Apis.slice(0, 10).map((value: Api, index: number) => (
                  <div className="group relative">
                    <a href="#" className="text-sm lg:w-full sm:w-8 w-full rounded-lg flex items-center gap-1 sm:h-8 h-10 group-hover:bg-gray-200 dark:group-hover:bg-gray-700 py-1">
                      <span className="flex size-8 shrink-0 items-center justify-center">
