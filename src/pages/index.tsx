      ...
      <div className={"h-full transition-[width] duration-100 ease-in-out sm:sticky sm:left-0 sm:top-0 sm:w-16 xl:sidebar-anchored:w-[240px] xl:sidebar-floating:w-16 hidden sm:block " + (open ? "xl:w-[240px]" : "xl:w-16")}>
        <nav className="bg-gray-100 dark:bg-gray-800 sm:z-[2] w-full fixed h-full sm:h-dvh translate-x-full sm:translate-x-0 sm:sticky sm:top-0 sm:left-0">
          <div className="flex h-full flex-col justify-stretch">
            ...
            <SimpleBar className="relative flex-1 pl-4 pr-2" style={{ height: 1 }}>
                ...
                <div style={{ minWidth: "100%", display: "table" }}>
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    {Apis.map((value: Api, index: number) =>
                      ...
                      //     <p className="text-[12px] text-center">{value.title}</p>
                      //   </Link>
                      // </li>

                      <a key={index} aria-label="Home" data-state="closed" className={"bg-surface-2 flex h-10 items-center gap-1 rounded-lg text-sm hover:bg-gray-200 dark:hover:bg-gray-700 sm:h-8 w-full sm:w-8" + (open ? " xl:w-full" : " justify-center")} href="#">
                        <div className="flex w-8 items-center justify-center">
                          {/* <Icon name={m} size={16} /> */}
                          <Image src={value.icon} alt="" width={16} height={16} />
                        </div>
                        {open && (
                          <span className="inline sm:hidden sm:sidebar-floating:hidden xl:inline">{value.title}</span>
                        )}
                      </a>
                    )}
                  </div>
            ...
            <div className="sticky bottom-0 z-10 mt-auto flex flex-col gap-4 p-4">
              ...
              <div className={"flex items-center justify-between gap-6 flex-col " + (open ? "xl:flex-row" : "xl:flex-col")}>
                <div className={"flex gap-2 flex-col " + (open ? "xl:flex-row" : "xl:flex-col")}>
                  <a className="p-[9px] hover:bg-gray-200 dark:hover:bg-gray-700 rounded-lg flex items-center cursor-pointer no-underline visited:text-inherit" href="/login">
                    <Settings size={16} />
                  </a>
                  <a className="p-[9px] hover:bg-gray-200 dark:hover:bg-gray-700 rounded-lg flex items-center cursor-pointer no-underline visited:text-inherit" href="/login">
                ...
                <button className="p-[9px] hover:bg-gray-200 dark:hover:bg-gray-700 rounded-lg flex items-center cursor-pointer no-underline visited:text-inherit" type="button" aria-haspopup="dialog" aria-expanded="false" aria-controls="radix-:R2lm9kq6:" data-state="closed">
