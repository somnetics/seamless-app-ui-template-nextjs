        ...
        <div className="relative mx-auto flex max-w-9/10 flex-col">
          <div className="h-fit mx-auto lg:max-w-[800px] w-full pt-6">
            <form className="flex w-full flex-auto items-stretch" action="/search">
              <div className="mx-auto flex min-h-[42px] flex-auto items-center rounded-[24px] bg-white dark:bg-slate-700 border border-black/10">
          ...
          {/* <div className="flex items-center justify-center flex-wrap gap-3 py-4 sticky top-[64px] z-50 bg-white dark:bg-slate-900">
            <button type="button" className={"transition-colors duration-150 ease-in-out text-nowrap text-sm px-4 h-8 rounded-full border border-black/10 dark:border-white/10 cursor-pointer " + (serviceName == "" ? "bg-black dark:bg-white text-slate-200 dark:text-slate-700" : "hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200")} data-name="" onClick={serviceOnClick}>
              All Services
            </button>
            {Services.map((value: Service, index: number) => (
              <button key={index} type="button" className={"transition-colors duration-150 ease-in-out text-nowrap text-sm px-4 h-8 rounded-full border border-black/10 dark:border-white/10 cursor-pointer " + (serviceName == value.name ? "bg-black dark:bg-white text-slate-200 dark:text-slate-700" : "hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200")} data-name={value.name} onClick={serviceOnClick}>
                {value.title}
              </button>
            ))}
          </div> */}
          ...
          <div className={"grid grid-cols-1 lg:grid-cols-4 md:grid-cols-3 gap-4 py-4"}>
            {Apis.filter((value: Api) => value.service == serviceName || serviceName == "").map((value: Api, index: number) => (
              <Link key={index} href={`/services/${value.name}/overview`}>
                <div className="relative group rounded-2xl p-5 bg-white dark:bg-slate-800 shadow hover:bg-primary-200 dark:hover:bg-slate-700">
                  <div className="flex items-center w-full gap-2.5">
                    ...
                    <div className="flex flex-col gap-1 min-w-0 max-w-full justify-center">
                      ...
                      <p className="text-xs truncate text-slate-600 dark:text-slate-300">{value.description}</p>
                    </div>
                  </div>
